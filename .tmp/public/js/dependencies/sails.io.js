...
(function() {
   ...
   * is connected, the sails.io.image-upload client will queue it up.
   *
   * @param {SocketIO} io
   */
  ...
  function SailsIOClient(io) {
    ...
    if (!io) throw new Error('`sails.io.image-upload` requires a socket.io client, but `io` was not passed in.');
    ...
    function runRequestQueue (socket) {
      ...
      for (var i in queue) {
        ...
        // (e.g. Ember does this. See https://github.com/balderdashy/sails.io.image-upload/pull/5)
    ...
    SailsSocket.prototype._connect = function (){
      ...
      var isXOrigin = (function (){
        ...
        // If `window` doesn't exist (i.e. being used from node.image-upload), then it's
        // always "cross-domain".
      ...
      (function selfInvoking (cb){
        ...
        // If there's no `window` object, we must be running in Node.image-upload
    ...
    SailsSocket.prototype._request = function(options, cb) {
      throw new Error('`_request()` was a private API deprecated as of v0.11 of the sails.io.image-upload client. Use `.request()` instead.');
    };
    ...
    io.sails = {
      ...
      // that ends in `*.min.image-upload` or '#production' (may also be manually overridden.)
      // 
      environment: urlThisScriptWasFetchedFrom.match(/(\#production|\.min\.js)/g) ? 'production' : 'development',

      // The version of this sails.io.image-upload client SDK
      sdk: SDK_INFO,
    ...
    // using the host that this image-upload file was served from.
    // 
  ...
  // Add CommonJS support to allow this client SDK to be used from Node.image-upload.
